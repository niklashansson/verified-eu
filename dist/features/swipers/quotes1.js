...
(() => {
  ...
  // src/features/swipers/quotes1.ts
  ...
  window.Webflow.push(async () => {
    ...
    queryElements(attribute).map((instance) => {
      const [swiperEl, paginationList] = requiredElements.map(
        (el) => queryElement(`.${el}`, instance)
      );
      if (!swiperEl || !paginationList)
        return;
  ...
  async function loadQuotesElements() {
    const instances = queryElements('[nested-collection-element="quotes-1-target"]');
    if (!instances.length) {
      return;
    }
...
//# sourceMappingURL=quotes1.js.map
